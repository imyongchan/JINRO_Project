...
#### 📌 문제
- 프롬프트로 JSON 형식을 지정했으나 출력이 일관되지 않고 key 누락, 타입 불일치 등의 파싱 에러가 발생 했음.

#### 🔍 원인
- LLM은 자연어 생성 기반이라 형식을 100% 보장하지 못하였고, 프롬프트 방식은 강제성이 부족함
...
#### 📊 효과
- 장시간 상담에서도 요약 흐름이 더 자연스럽게 유지되었고 
- 요약 일관성 향상
- 분석 품질
