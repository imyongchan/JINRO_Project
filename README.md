...
#### 📌 문제
- 집중/비집중 라벨 데이터가 불균형하여 모델이 다수 클래스로 편향 예측, 정확도는 높지만 재현율이 낮은 결과 발생

#### 🔍 원인
- 비집중 클래스의 학습 데이터가 부족해 모델이 집중 클래스 위주로 학습
  
#### 💡 해결 방법
- Yarning Detection 데이터셋의 Yarning 데이터를 추가하여 비집중 부분 데이터를 보강하고 데이터 밸런스를 맞춤
- 가중치 손실(`CrossEntropyLoss(weight=...)`)을 적용하여 균일 학습을 유도
...
#### 📊 효과
- loss 0.6141 → 0.2532, acc 0.756 → 0.583 
- Recall 0.84 → 0.91, F1 Score 0.82 → 0.88 향상
...
#### 📌 문제
- 영상 전체 프레임을 분석하면서 GPU OOM과 응답 지연이 발생, 동시 분석 요청이 겹치면 파이프라인이 중단될 위험 존재

#### 🔍 원인
- 동시 작업 제한 없이 GPU 메모리를 점유하는 구조
- 매 프레임마다 모델 추론을 수행하여 불필요한 연산량 발생

#### 💡 해결 방법
-5프레임 간격으로 샘플링 
-Semaphore로 GPU 동시 접근 제한
...
#### 📊 효과
- GPU OOM 에러 제거, 평균 분석 지연 40% 감소

---
### 3️⃣ LLM Hallucination (환각) 문제

#### 📌 문제
- 상담 음성을 STT로 변환한 뒤 LLM으로 요약할 때, 긴 대화를 한 번에 처리하면 문맥이 섞이거나 근거 없는 추천 문장이 섞일 가능성
...
accumulated_summary = ""
for i, chunk in enumerate(chunks): #이전 요약을 다음 chunk에 누적하며 순차 갱신
...
---
### 4️⃣ 서버 구조 분리 및 운영 안정화

#### 📌 문제
- AI 모델 추론과 API 요청이 하나의 서버에서 처리되면서 응답 지연 발생
- AI Server의 영상·음성 데이터가 Public IP로 외부에 노출될 수 있는 보안 위험 존재
- 로그가 무제한 누적되면서 디스크 용량 부족 및 에러 추적 어려움

#### 🔍 원인
- Backend(API, DB)와 AI 연산이 동일 서버에서 동작해 연산 작업이 I/O 응답에 영향
- 서버 간 통신이 Public IP 기반으로 설계되어 외부 접근 가능성 존재
- 로그 관리 정책 부재로 파일이 무한 누적
...
#### 로그 자동 관리 — TimedRotatingFileHandler 적용```python
```
...
#### 📊 효과
- AI 연산과 서비스 로직 분리로 API 응답 속도 안정화, 병목 제거
- 외부에서 AI Server 직접 접근 차단, 민감 데이터 보호
- 날짜 기준 로그 자동 분리 및 삭제로 디스크 안정화, 시점별 에러 추적 가능
